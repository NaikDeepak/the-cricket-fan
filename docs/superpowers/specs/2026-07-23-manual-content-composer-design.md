...
| `POST /generate/bot` | Body `{kind}` (prediction/trivia/h2h/venue/record) → generated text (+ card_meta) via the bot generators. Creates a `draft` (source=`bot`), logs `generated`. |
| `POST /generate/llm` | Body `{prompt, category}` → Gemini text. Creates a `draft` (source=`llm`), logs `generated`. |
...
2. **Editor** — textarea with a live **280-char counter** (soft warning past
   ...
   card-type selector. Autosaves via `PATCH /drafts/{id}`.
...
render path. Handle web-font readiness (`document.fonts.ready`) before snapshot
to avoid the known font-swap glitch. Aspect ratio = the card node's fixed
dimensions per selection (1080×1080, 1200×675, 1080×1350).
...
- `/generate/bot` returns text for each kind and creates a `draft` + a
  `generated` event (bot generators exercised, not mocked).
- `/generate/llm` with the Gemini client mocked: creates an `llm` draft; and
  with no key, returns a clear disabled response (no crash).
- drafts CRUD: create/list/patch; `edited` event on patch.
- `/drafts/{id}/event`: `copied` logs an event; `posted` also flips status +
  sets `posted_at`.
- `/analytics`: funnel aggregation over a mix of events, and the prediction
  record matches `_season_record` on seeded outcomes.
